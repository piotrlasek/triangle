...
public abstract class NBCBase implements IClusteringAlgorithm {
    ...
    public void run2() {
         ...
         * addLines();
         * 
         * for (int i = 0; i < dataset.size(); i++) { NbcSpatialObject p =
         * (NbcSpatialObject) dataset.get(i); ArrayList<ISpatialObject> kNBp =
         * (ArrayList<ISpatialObject>)lva.getNeighbors(p, k, 600);
         * 
         * System.out.print( p + "\t" + kNBp.size() + "\t"); System.out.print(
         * p.SizeOfRkNB + "/" + p.SizeOfkNB + "\t");
         * 
         * 
         * if (kNBp.size() < k-1 ) { System.out.print(" ! "); } else {
         * System.out.print("   "); }
         * 
         * for(int j = 0; j < kNBp.size(); j++) { NbcSpatialObject q =
         * (NbcSpatialObject) kNBp.get(j); if ( q != null ) { double x2 =
         * p.getCoordinates()[0] - q.getCoordinates()[0]; double y2 =
         * p.getCoordinates()[1] - q.getCoordinates()[1]; double dist =
         * Math.sqrt( x2*x2 + y2*y2 ); System.out.print( "\t" + (float) dist );
         * } else { observer.handleNotify("ble"); } } }
         */
    }
