...
public class TriangleIndexSqrEpsRefNoSqrt extends LoggingIndex implements lvaindex.vafile.ISpatialIndex {
     ...
     * @param k
     */
    public TriangleIndexSqrEpsRefNoSqrt(int k) {
    ...
    public void Verify_k_Candidate_Neighbours_Backward(
        ...
        while (backwardSearch == 1 &&
                (Math.pow(p.Dists.get(0) - b[0].Dists.get(0), 2) <= p.Eps2)) {
            boolean candidateNeighbor = true;
            int ii = 1;
            while (candidateNeighbor && (ii < p.Dists.size())) {
                if (Math.pow(p.Dists.get(ii) - b[0].Dists.get(ii), 2) > p.Eps2) {
    ...
    public void Verify_k_Candidate_Neighbours_Forward(
        ...
        while (forwardSearch == 1 && (Math.pow(f[0].dist - p.dist, 2) <= p.Eps2)) {
            boolean candidateNeighbor = true;
            int ii = 1;
            while (candidateNeighbor && (ii < p.Dists.size())) {
                if (Math.pow(f[0].Dists.get(ii) - p.Dists.get(ii), 2) > p.Eps2) {
                    candidateNeighbor = false;
                } else {
                    ii = ii + 1;
                }
